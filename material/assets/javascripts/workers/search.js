...
(() => {
  ...
  // src/assets/javascripts/integrations/search/internal/_/index.ts
  ...
  // src/assets/javascripts/integrations/search/internal/extract/index.ts
  ...
  // src/assets/javascripts/integrations/search/internal/highlight/index.ts
  function highlight(input, table, positions) {
    return highlightAll([input], table, positions).pop();
  }
  function highlightAll(inputs, table, positions) {
    const mapping = [0];
  ...
  // src/assets/javascripts/integrations/search/internal/tokenize/index.ts
  ...
  function transform(query, fn = (term) => term) {
    ...
    ).join("").replace(/"|(?:^|\s+)[*+\-:^~]+(?=\s+|$)/g, "").split(/\s+/g).flatMap(fn).map((term) => /([~^]$)/.test(term) ? `${term}1` : term).map((term) => /(^[+-]|[~^]\d+$)/.test(term) ? term : `${term}*`).join(" ");
  }
